...
Test run parallel (takes ~ 1 minute):
run_lsff -m 5 -r 6:00:00 --num_simulants 100 --draws 10 --random_seed 3456 --take_mean False  `pwd`/iron_bw_calc_industry.py /ihme/homes/ndbs/vivarium_research_lsff `pwd`/parallel_test2

Actual run parallel (takes ~ 2 hours):
run_lsff -m 50 -r 6:00:00 --num_simulants 80_000 --draws 250 --random_seed 3456 --take_mean False  `pwd`/iron_bw_calc_industry.py /ihme/homes/ndbs/vivarium_research_lsff `pwd`/results_industry
'''
...
def get_name_and_id_for_location(location_df, location):
    ...
    if isinstance(location, int) or (isinstance(location, str) and location.isdigit()):
        ...
        location_id = int(location_df.set_index('location_name').loc[location_name, 'location_id'])
    else:
...
#     year = 2025 # Needed in index for Ali's code
    ...
    # TODO: Oh wait, I should be able to use fortification_input_data.locations instead of reading it here
    ...
    # Specify coverage data path with industry wheat
    fortification_input_data = data_processing.get_fortification_input_data(
        vivarium_research_lsff_path,
        coverage_data_path=f'{vivarium_research_lsff_path}/nanosim_models'
            '/results_industry/lsff_input_coverage_data_w_industry_oil_wheat_v4_04_15_2021.csv'
    )
...
#     vehicles_for_location = fortification_input_data.consumption.query("location_name==@location")['vehicle'].unique()
    ...
    for vehicle in vehicles_for_location:
        # Only process industry wheat data
        if vehicle != 'industry wheat':
            continue
...
#         # Note: target_coverage is population coverage proportion
...
#         ]
        ...
        proportions = gap_coverage_proportions + additional_proportions
